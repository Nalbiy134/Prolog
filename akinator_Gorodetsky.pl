...
problem(keys_mouse) :-
    query('nothing happens when I press a mouse button'). %при нажатии клавишь на клавишу мыши ничего не происходит
problem(mouse_connector) :-
    query('mouse connector connected to computer'). %подключен разьём мыши в компьютер
problem(mouse_does_not_work) :-
    query('when I restart the computer the mouse still does not work'). %при перезагрузке компьютера мышь все равно не работает


%моя проблема
fault(mouse is broken) :-  %мышь сломана
    problem(keys_mouse),
    problem(mouse_connector),
    problem(mouse_does_not_work).
